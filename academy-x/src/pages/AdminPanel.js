...
const AdminPanel = ({ setView, features, setFeatures }) => {
    ...
    const handleGenerateQuiz = async () => {
        ...
        console.log("Simulating API call to secure backend with text:", quizText);
        setTimeout(() => {
            const mockResponse = {
                title: "Generated Quiz from Text",
                accessCode: `AI${Date.now().toString().slice(-4)}`,
                questions: [
                    { id: 1, text: "What was the main topic?", options: ["A", "B", "C", "D"], answer: "A" },
                    { id: 2, text: "What was a key concept?", options: ["X", "Y", "Z", "W"], answer: "Y" },
                ]
            };
            setGeneratedQuiz(mockResponse);
